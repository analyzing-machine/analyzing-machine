...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Analyzing-Machine thinks for you about the meanings.",
        epilog="Example:  analyzing-machine input_text.txt > output_text.txt"
...
def run():
    """
    $ text | analyzing-machine                        # Accepts text from the pipe
    $ echo "...<text>..." | analyzing-machine         #

    $ analyzing-machine multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="analyzing-machine",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"analyzing-machine: {utterance}.")
        syslog.closelog()
